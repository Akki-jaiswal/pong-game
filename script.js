...
function resetBall() {
  ...
  ballSpeedX =
    (Math.random() > 0.5 ? 1 : -1) * currentSettings.ballInitialSpeed;
...
function moveEverything() {
  ...
  if (ballX < 0) {
    ...
    if (
      aiScore >= minimumWinningScore &&
      aiScore - playerScore >= scoreDifferenceToWin
    ) {
      gamePaused = true;
  ...
  if (ballX > canvas.width) {
    ...
    if (
      playerScore >= minimumWinningScore &&
      playerScore - aiScore >= scoreDifferenceToWin
    ) {
      gamePaused = true;
  ...
  if (aiPaddleY + paddleHeight > canvas.height)
...
pauseButton.addEventListener("click", () => {
  if (!countdownActive) {
    // Only allow pause/resume when not in active countdown
    if (gamePaused) {
      // If currently paused, user wants to resume
      startBackgroundMusicRotation();
      gamePaused = false;
      pauseButton.textContent = "Pause";
      if (!animationFrameId) {
      ...
      // If currently playing, user wants to pause
      gamePaused = true;
      pauseButton.textContent = "Resume";
      if (animationFrameId) {
        cancelAnimationFrame(animationFrameId);
...
document.addEventListener("keydown", function (e) {
  if (e.code === "Space") {
    e.preventDefault(); // Prevent default page scrolling on spacebar keypress
